...
void build_from_matrix_helper(const K2_tree& mat, int size, int thrds, int number_children, int power)
{
    ...
    ofstream K_T ("./outputs/KT_1.sdsl"), K_L ("./outputs/KL_1.sdsl");
    k_t.resize(t);
    k_l.resize(l);
...
int main(int argc, char* argv[])
{
    ...
    auto time = average_time(build_from_matrix_helper, matrix, n, atoi(argv[2]));

    cout << "Build from Matrix Parallel time = " << time << "\n" <<endl;
