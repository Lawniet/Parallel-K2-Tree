...
void build_from_matrix(const K2_tree& mat, int size, int number_children, int power)
{
    ...
    ofstream K_T ("./outputs/KT_6.sdsl"), K_L ("./outputs/KL_6.sdsl");
    k_t.resize(t);
    k_l.resize(l);
...
int main(int argc, char* argv[])
{
    ...
    auto time_par_3 = average_time(build_from_matrix, matrix, n, true);

    cout << "Build from Matrix Sequential (with Tiling and Unrolling Loop) time = " << time_par_3 << "\n" <<endl;
