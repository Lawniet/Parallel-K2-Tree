...
void build_from_matrix(const K2_tree& mat, int size, int thrds, int number_children, int power)
{
    ...
    ofstream K_T ("./outputs/KT_5.sdsl"), K_L ("./outputs/KL_5.sdsl");
    k_t.resize(t);
    k_l.resize(l);
    ...
    /*cout <<" Bit  vector T: ";
    ifstream KT_in("KT_4.sdsl");
    sdsl::bit_vector kt;
    load(kt, KT_in);
    cout << kt << endl;
    cout << "\n Bit vector L: ";
    ifstream KL_in("KL_4.sdsl");
    sdsl::bit_vector kl;
    load(kl, KL_in);
    cout << kl << endl;*/
}
...
int main(int argc, char* argv[])
{
    ...
    cout << "Build from Matrix Parallel (with Unrolling Loop) time = " << time << "\n" <<endl;
