...
double average_time(Fun&& func, const K2_tree& mat, int n)
{
...
int build_from_matrix_helper_sequential(const K2_tree& mat, k2_tree_node& matrix, int n)
{
    ...
    int found = 0;
    for (int i = min_size_i; i < max_size_i; i++)
    {
        for (int j = min_size_j; j < max_size_j; j++)
        {
            if (mat(i, j) == 1)
            {
                found = 1;
...
void build_from_matrix(const K2_tree& mat, int size, int number_children, int power)
{
    ...
    while (!q.empty())
    {
        ...
        if(matrix_size == 1) // Leaf level base case, must be stored in bit vector L
        {
            ...
            break;
        else if (size == matrix_size) // Root of the tree, must not be stored
            build_from_matrix_helper_sequential(mat,matrix,matrix_size);	
        else // Internal nodes, must be stored in bit vector T
        {
    ...
    ofstream K_T ("./outputs/KT_0.sdsl"), K_L ("./outputs/KL_0.sdsl");
    k_t.resize(t);
    k_l.resize(l);
...
int main(int argc, char* argv[])
{
    ...
    auto time = average_time(build_from_matrix, matrix, n);

    cout << "Build from Matrix Sequential time = " << time << "\n" <<endl;
